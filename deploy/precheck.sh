...
while read -r val; do
    [ -z "$val" ] && continue
    # 簡化邏輯：利用 git ls-files 僅檢查受版本控制的檔案 (排除常見檔案)
    if git ls-files | grep -vE 'package-lock.json|App.test.js' | xargs grep -lF "$val" | grep -q .; then
        echo "❌ 錯誤：偵測到硬編碼敏感資訊 \"$val\" 存在於以下檔案中："
        git ls-files | grep -vE 'package-lock.json|App.test.js' | xargs grep -lF "$val"
        exit 1
    fi
done <<< "$ENV_VALUES"
# --- 禁止修改：檢查硬編碼變數結束 ---
